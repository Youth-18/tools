...
void train_detector(char *datacfg, char *cfgfile, char *weightfile, int *gpus, int ngpus, int clear)
{
    ...
    while(get_current_batch(net) < net->max_batches){
     ...
     //   float rate1=get_current_rate(net);
...
void validate_detector_recall(char *cfgfile, char *weightfile)
{
    ...
    list *plist = get_paths("/data/datahead/test.txt");                          //生成的test.txt路径
    char **paths = (char **)list_to_array(plist);
    ...
    float total_recall=0.00;
    float total_precision=0.00;
    float total_accuracy=0.00;
    float thresh = .001;
    float iou_thresh = .5;
    float nms = .4;
    ...
    for(i = 0; i < m; ++i){
        ...
        //IplImage *pImg = cvLoadImage(path,1);
        //cvNamedWindow("recall",1);
        ...
        for(k = 0; k < nboxes; ++k){
            if(dets[k].objectness > thresh && dets[k].bbox.x<=1 && dets[k].bbox.y<=1 && dets[k].bbox.w<=1 && dets[k].bbox.h<=1 ){
                ++proposals;
            }
        }
        for (j = 0; j < num_labels; ++j) {
            ...
            for(k = 0; k < l.w*l.h*l.n; ++k){
                ...
                //fprintf(stderr, "l.w:%5d\t l.h:%5d\t l.n:%5d\n",l.w,l.h,l.n );
                if(dets[k].objectness > thresh && iou > best_iou && dets[k].bbox.x<=1 && dets[k].bbox.y<=1 && dets[k].bbox.w<=1 && dets[k].bbox.h<=1 ){
                    best_iou = iou;
                    //cvRectangle(pImg,cvPoint((dets[k].bbox.x-dets[k].bbox.w/2+1),(dets[k].bbox.y-dets[k].bbox.h/2+1)),cvPoint((dets[k].bbox.x+dets[k].bbox.w/2),(dets[k].bbox.y+dets[k].bbox.h/2)),cvScalar(255,255,255),1,1,0);
                    //fprintf(stderr, "k:%d\t dets.bbox.x:%f\t dets.bbox.y:%f\t dets.bbox.w:%f\t dets.bbox.h:%f\n",k,dets[k].bbox.x,dets[k].bbox.y,dets[k].bbox.w,dets[k].bbox.h);
                    //fprintf(stderr, "j:%df\t t.x:%f\t t.y:%f\t t.w:%f\t t.h:%f\n",j,t.x,t.y,t.w,t.h);
                    //fprintf(stderr, "iou:%f\t best_iou:%f\n",iou,best_iou );
                    //count1++;
                    //count2++;
            ...
            //fprintf(stderr, "Avg_iou:%f\n",avg_iou );
            if(best_iou > iou_thresh){
                ++correct;
            }    
        }
        //fprintf(stderr, "count1:%d\n",count1 );
        //cvShowImage("recall",pImg);
        //cvWaitKey(0);
        //cvReleaseImage(&pImg);
        //cvDestroyWindow("recall");
        total_recall = total_recall + 100.*correct/total;
        total_precision = total_precision + 100.*correct/(float)proposals;
        total_accuracy = total_accuracy + avg_iou*100/total;
        fprintf(stderr, "ID:%5d Correct:%5d Total:%5d\t RPs/Img: %.2f\tIOU: %.2f%%\tRecall:%.2f%%\n", i, correct, total, (float)proposals/(i+1), avg_iou*100/total, 100.*correct/total);
        fprintf(stderr, "proposals:%5d\tPrecision:%2f%%\nTotal_Recall:%2f%%\nTotal_Precision:%2f%%\n", proposals, 100.*correct/(float)proposals, total_recall, total_precision);
...
void calc_anchors(char *datacfg, int num_of_clusters, int width, int height, int show)
{
    ...
    for (i = 0; i < number_of_boxes; ++i) {
        ...
        if (iou > 1 || iou < 0) {
            printf(" i = %d, box_w = %f, box_h = %f, anchor_w = %f, anchor_h = %f, iou = %f \n",
...
void run_detector(int argc, char **argv)
{
    ...
    int width1 = find_int_arg(argc, argv, "-width", -1);
    int height1 = find_int_arg(argc, argv, "-height", -1);
    /*-----------------------add------------------------------*/
    ...
    /*---------------------------------add----------------------------------------------------------*/
    else if(0==strcmp(argv[2], "calc_anchors")) calc_anchors(datacfg, num_of_clusters, width1, height1, show);
    /*---------------------------------add----------------------------------------------------------*/
